...
def cleanup_contacts(input_file, output_file):
    ...
    # Apply the cleanup_email function to the 'Shipper Email 1' column
    data['Shipper Email 1'] = data["Shipper Email 1"].apply(cleanup_email) 

    # Select only the desired columns (Shipper name and Shipper email 1)
    cleaned_data = data[['Shipper Name', 'Shipper Email 1']]

    # Display the first few rows of the cleaned data
    print("\nCleaned Data:")
    print(cleaned_data.head())

    # Drop rows with missing values in either Shipper name or Shipper email 1
    cleaned_data = cleaned_data.dropna(subset=['Shipper Name', 'Shipper Email 1'])
